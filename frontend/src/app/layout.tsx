...
export default function RootLayout({
  ...
  const isCRMRoute = pathname?.startsWith('/crm') || pathname?.startsWith('/encuestas');
  const isRRHHRoute = pathname?.startsWith('/rrhh');
  const isSuperAdminRoute = pathname?.startsWith('/super-admin');
  const isStandaloneRoute = isLandingPage || isCRMRoute || isRRHHRoute || isSuperAdminRoute;
    ...
    <html lang="es">
      <body className={inter.className}>
        <DonCandidoProvider position="bottom-right" size="lg">
          <ToastProvider>
          {isStandaloneRoute ? (
            // Layout para Landing y CRM (sin sidebar principal ni header)
