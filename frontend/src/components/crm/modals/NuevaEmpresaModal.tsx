...
import type { CreateCRMClienteDTO } from '@/types/crm';
...
export default function NuevaEmpresaModal({ 
  ...
  const [formData, setFormData] = useState<CreateCRMClienteDTO>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await crmClienteService.create(formData);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Nueva Empresa</DialogTitle>
        </DialogHeader>
