...
import type { CreateCRMActividadDTO } from '@/types/crm';
...
export default function NuevaActividadModal({ 
  ...
  const [formData, setFormData] = useState<CreateCRMActividadDTO>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await crmActividadService.create(formData);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Nueva Actividad</DialogTitle>
        </DialogHeader>
