...
import type { UpdateCRMOportunidadDTO } from '@/types/crm';

interface EditarOportunidadModalProps {
  isOpen: boolean;
  onClose: () => void;
  oportunidad: any;
  onSuccess?: () => void;
...
export default function EditarOportunidadModal({ 
  isOpen, 
  onClose, 
  oportunidad,
  onSuccess 
}: EditarOportunidadModalProps) {
  const { organizationId } = useOrganization();
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState<UpdateCRMOportunidadDTO>({
    titulo: '',
    descripcion: '',
    valor_estimado: 0,
    moneda: 'MXN',
  ...
  useEffect(() => {
    if (isOpen && oportunidad) {
      loadData();
      populateForm();
    }
  }, [isOpen, oportunidad, organizationId]);
  ...
  const populateForm = () => {
    if (oportunidad) {
      setFormData({
        ...
        productos_incluidos: oportunidad.productos_incluidos || [],
    ...
    try {
      ...
      if (oportunidad?.cliente_id) {
        const contactosRes = await crmContactoService.getAll(organizationId, { 
          empresa_id: oportunidad.cliente_id, 
          limit: 100 
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await crmOportunidadService.update(oportunidad.id, formData);
      ...
      if (response.success) {
        ...
        toast.error(response.message || 'Error al actualizar la oportunidad');
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Editar Oportunidad</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end space-x-4 pt-6 border-t">
            ...
            >
              ...
              {loading ? 'Actualizando...' : 'Actualizar Oportunidad'}
            </Button>
          </div>
        </form>
      </DialogContent>
