...
import type { CreateCRMContactoDTO } from '@/types/crm';
...
export default function NuevoContactoModal({ 
  isOpen, 
  onClose, 
  onSuccess 
}: NuevoContactoModalProps) {
  const { organizationId } = useOrganization();
  const [loading, setLoading] = useState(false);
  const [clientes, setClientes] = useState<any[]>([]);
  const [formData, setFormData] = useState<CreateCRMContactoDTO>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await crmContactoService.create(formData);
      ...
      if (response.success) {
        ...
        toast.error(response.message || 'Error al crear el contacto');
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Nuevo Contacto</DialogTitle>
        </DialogHeader>
