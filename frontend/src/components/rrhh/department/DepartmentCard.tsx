      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-semibold text-lg">
            {getInitials(department.nombre)}
          </div>
          <div>
            <h3 className="font-semibold text-gray-900">{department.nombre}</h3>
            <p className="text-sm text-gray-600">{department.gerente_nombre || 'Sin gerente asignado'}</p>
            <p className="text-xs text-gray-500 font-mono">ID: {department.id}</p>
          </div>
        </div>
      </div>

      <p className="text-sm text-gray-600 mb-4 line-clamp-2">{department.descripcion || 'Sin descripción'}</p>

      <div className="space-y-2 text-sm text-gray-500 mb-4">
        <div className="flex items-center gap-2">
          <Users className="h-4 w-4" />
          <span>{department.cantidad_empleados || 0} empleados</span>
        </div>
        <div className="flex items-center gap-2">
          <UserCheck className="h-4 w-4" />
          <span>Gerente: {department.gerente_nombre || 'Sin asignar'}</span>
        </div>
        <div className="flex items-center gap-2">
          <DollarSign className="h-4 w-4" />
          <span>Presupuesto: {formatBudget(department.presupuesto)}</span>
        </div>
        {department.objetivos && (
          <div className="flex items-center gap-2">
            <Target className="h-4 w-4" />
            <span className="line-clamp-1">{department.objetivos}</span>
          </div>
        )}
      </div>

      <div className="flex flex-wrap gap-2 mb-4">
        <Badge className={getEstadoColor(department.estado)}>
          {department.estado}
        </Badge>
        <Badge variant="outline">
          {department.cantidad_empleados || 0} empleados
        </Badge>
      </div>
