...
exports.seedNormPoints = seedNormPoints;
...
async function seedNormPoints() {
    try {
        console.log('🌱 Iniciando seeding de puntos de norma ISO 9001...');
        const existingCount = await NormPoint_1.NormPoint.countDocuments();
        if (existingCount > 0) {
        ...
        const insertedPoints = await NormPoint_1.NormPoint.insertMany(normPointsWithUser);
        console.log(`✅ Seeding completado exitosamente. Insertados ${insertedPoints.length} puntos de norma.`);
        const chapterSummary = await NormPoint_1.NormPoint.aggregate([
...
exports.default = seedNormPoints;
//# sourceMappingURL=normPointsSeeder.js.map